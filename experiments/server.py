"""Minimal dataset-scoped HTTP adapter for OmniMemEval."""
...
def main():
    ...
    if len(token) < 24:
        raise SystemExit('Set HESM_EVAL_TOKEN in the experiment env to at least 24 characters')
    # Production logging configuration is never initialized.
    logging.basicConfig(level=logging.WARNING)
    import uvicorn
    uvicorn.run(create_app(EvaluationBackend(settings, args.dataset_id), token),
